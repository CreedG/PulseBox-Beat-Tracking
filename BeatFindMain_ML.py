...
def ml():
    ...
    while (True):
        ...
        increasing = True
        while (increasing):
            save = ALL_PARAMETERS[cur_param]
            change_param(cur_param,1.3)
            print("Running session with param",cur_param,"increased to val",ALL_PARAMETERS[cur_param])
            score = session()
            if (score > cur_max):
                print("Increasing", cur_param, "gave new best", score)
                print("Params used",ALL_PARAMETERS)
                cur_max = score
            else:
                ALL_PARAMETERS[cur_param] = save
                increasing = False
        ...
        while (decreasing):
            save = ALL_PARAMETERS[cur_param]
            change_param(cur_param, 0.7)
            print("Running session with param", cur_param, "decreased to val", ALL_PARAMETERS[cur_param])
            score = session()
            if (score > cur_max):
